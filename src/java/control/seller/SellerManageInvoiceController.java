 ...
 * @author LENOVO
 */
@WebServlet(name = "SellerManageInvoiceController", urlPatterns = {"/seller/manageInvoice"})
public class SellerManageInvoiceController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Invoice_Status> listSta = sellerDao.findAllInvoiceStatus();
        ...
        switch (page) {
            case "view-invoice-details":
                ...
                request.getRequestDispatcher("/views/seller/page_Invoice.jsp").forward(request, response);
                break;
            default:
                session.setAttribute("listStatus", listSta);
                session.setAttribute("listIf", listIf);
                request.setAttribute("pagination", pagination);
                request.getRequestDispatcher("/views/seller/invoiceList.jsp").forward(request, response);
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (action) {
            case "delete-invoice":
                ...
                url = "/views/seller/invoiceList.jsp";
                break;
            case "update-status":
                String invoiceIdRaw = request.getParameter("invoiceIdToUpdate");
                String statusIdRaw = request.getParameter("status");
                if (invoiceIdRaw == null || invoiceIdRaw.isEmpty() || statusIdRaw == null
                        || statusIdRaw.isEmpty()) {
                    request.setAttribute("errorei", "Fail when update status!");
                } else {
                    int invoiceID = Integer.parseInt(invoiceIdRaw);
                    int statusID = Integer.parseInt(statusIdRaw);
                    int rowCount = sellerDao.updateInvoiceStatus(invoiceID, statusID);
                    String status = "";
                    if (statusID == 1) {
                        status = "Pending";
                    }
                    ...
                    Invoice_Form af = sellerDao.getAccountByInvoiceID(invoiceID);
                    String htmlContent = invoiceHTML(invoiceID ,status, request, response);
                    emailSender.sendMsgEmail(af.getEmail(), htmlContent, "Your Invoice");
                    if (rowCount == 1) {
                        request.setAttribute("msgei", "Update status successfully!");
                    } else {
                        request.setAttribute("errorei", "Fail when update status!");
                    }
                }
    ...
    private List<Invoice_Form> pagination(HttpServletRequest request, Pagination pagination) {
        ...
        String status = request.getParameter("searchByStatus");
        if (status != null && !status.isEmpty()) {
            listIf = sellerDao.findTotalInvoice(status);
        } else {
    ...
    private void deleteInvoice(HttpServletRequest request, HttpServletResponse response) {
        ...
        sellerDao.deleteInvoiceByInvoiceID(invoiceID);
        if (sellerDao.findInvoiceDetailExist(invoiceID)) {
            request.setAttribute("errordi", "Fail when delete invoice!");
        } else {
            sellerDao.deleteInvoiceByID(invoiceID);
            request.setAttribute("msgdi", "Delete invoice successfully !!");
        }
    }

    private String invoiceHTML(int invoiceID, String sta, HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        HttpSession session = request.getSession();
        Invoice_Form af = sellerDao.getAccountByInvoiceID(invoiceID);
        String recipient = af.getUsername();
        String deliveryAddress = af.getAddress();
        String phone = af.getPhoneNumber();
